...
ドット絵向けのトークンは `core/designsystem` に集約してある。
寸法はすべて `PixelDimens.Unit`(4dp)の整数倍で、ドット絵を出すときは
`PixelImage`(`FilterQuality.None`)か `PixelCanvas` を通す。

画面はホーム / マップ / 達成記録 / 設定の4タブ。達成演出はタブの上に重ねて表示し、
バックグラウンドで貯まったぶんは次回起動時に順番に再生する。

### バックアップ

設定画面から記録をJSONへ書き出し／読み込みできる(仕様6.3)。
読み込みは上書きではなくマージで、同じ日の距離は多い方を採る。
同じファイルを何度読み込んでも距離は増えない。

### テスト

`./gradlew testDebugUnitTest` ですべて実行できる。Compose の UI テストも
Robolectric 上の単体テストとして置いてあるので、実機やエミュレータは要らない。

## ステータス

P5(UI・演出)、P7-1〜P7-5(JSONエクスポート/インポート)、P8-7(UIテスト)まで実装済み。
次は P6(アセット制作)。効果音・ドットフォント・ドット世界地図は P6 待ちで、現状は代替表示で動く。
クラウド同期(P7-6〜P7-9)は未実装。実機・エミュレータでの動作確認は未実施(P8 で行う)。
